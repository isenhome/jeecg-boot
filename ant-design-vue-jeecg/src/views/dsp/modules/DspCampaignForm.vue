        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="预算" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['limitCost', validatorRules.limitCost]" placeholder="请输入预算" style="width: 100%" />
            </a-form-item>
          </a-col>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'createBy','createTime','updateBy','updateTime','sysOrgCode','status','name','advertiserId','industryId','start','end','comment','limitCost'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'createBy','createTime','updateBy','updateTime','sysOrgCode','status','name','advertiserId','industryId','start','end','comment','limitCost'))
      },
    }
  }
</script>