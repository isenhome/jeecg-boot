        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="父级" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['parentId']" placeholder="请输入父级"  ></a-input>
            </a-form-item>
          </a-col>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'createBy','createTime','updateBy','updateTime','sysOrgCode','status','name','parentId'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'createBy','createTime','updateBy','updateTime','sysOrgCode','status','name','parentId'))
      },
    }
  }
</script>