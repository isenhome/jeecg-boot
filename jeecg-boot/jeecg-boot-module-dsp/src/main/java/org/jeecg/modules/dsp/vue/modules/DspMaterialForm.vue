        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="素材图片地址" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['imgUrl']" placeholder="请输入素材图片地址"  ></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'createBy','createTime','updateBy','updateTime','sysOrgCode','status','name','advertiserId','imgUrl','comment'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'createBy','createTime','updateBy','updateTime','sysOrgCode','status','name','advertiserId','imgUrl','comment'))
      },
    }
  }
</script>